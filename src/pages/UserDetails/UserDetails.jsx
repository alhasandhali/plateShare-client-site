...
const UserDetails = () => {
  ...
  const [recievedFood, setRecievedFood] = useState([]);
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(`http://localhost:3000/requested-foods?email=${res.data.email}`)
          .then((res) => setRecievedFood(res.data))
  ...
  return (
    <div className="py-10 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <section className="mt-10 lg:mt-16">
        <h2 className="text-2xl sm:text-3xl font-semibold text-gradient mb-6">
          {name}’s Received Foods:{" "}
          <span className="font-bold">{recievedFood.length}</span>
        </h2>
        <div className="overflow-x-auto rounded-lg border border-gray-300 shadow-md">
          <table className="min-w-full border-collapse text-sm sm:text-base">
            <thead className="bg-gray-100 text-gray-700">
